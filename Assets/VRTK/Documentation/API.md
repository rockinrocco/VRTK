 ...
 * **Pressed Distance:** The local space distance along the `Operate Axis` until the button reaches the pressed position.
 * **Stay Pressed:** If this is checked then the button will stay in the pressed position when it reaches the maximum position.
 * **Min Max Limit Threshold:** The threshold in which the button's current normalized position along the `Operate Axis` has to be within the minimum and maximum limits of the button.
 * **Resting Position:** The normalized position of the button between the original position and the pressed position that will be considered as the resting position for the button.
 * **Resting Position Threshold:** The normalized value that the button can be from the `Resting Position` before the button is considered to be resting when not being interacted with.
 * **Position Target:** The normalized position of the button between the original position and the pressed position. `0f` will set the button position to the original position, `1f` will set the button position to the pressed position.
 * **Target Force:** The amount of force to apply to push the Button towards the intended target position.
 ...
 * Parameters
   * _none_
 * Returns
   * `bool` - Returns `true` if the button is currently at the resting position.

The IsResting method returns whether the button is currently at it's resting position.
 ...
 * **Minimum Angle:** The minimum angle the door can rotate to.
 * **Maximum Angle:** The maximum angle the door can rotate to.
 ...
 * **Angle Target:** The target angle to rotate the door to.
 ...
 * **Detach Distance:** The maximum distance the grabbing object is away from the door before it is automatically released.
...
**Script Usage:**
 * Place the `VRTK_ArtificialButton` script onto the GameObject that is to become the button.
 ...
 * Parameters
   * `float newAngle` - The angle in which to rotate the door to.
 * Returns
   * _none_
