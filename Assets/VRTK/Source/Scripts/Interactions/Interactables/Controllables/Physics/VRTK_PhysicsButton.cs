...
namespace VRTK.Controllables.PhysicsBased
{
    ...
    public class VRTK_PhysicsButton : VRTK_BasePhysicsControllable
    {
        ...
        [Tooltip("The local space distance along the `Operate Axis` until the button reaches the pressed position.")]
        public float pressedDistance = 0.1f;
        [Tooltip("If this is checked then the button will stay in the pressed position when it reaches the maximum position.")]
        public bool stayPressed = false;
        [Tooltip("The threshold in which the button's current normalized position along the `Operate Axis` has to be within the minimum and maximum limits of the button.")]
        [Range(0f, 1f)]
        public float minMaxLimitThreshold = 0.01f;
        [Tooltip("The normalized position of the button between the original position and the pressed position that will be considered as the resting position for the button.")]
        [Range(0f, 1f)]
        public float restingPosition = 0f;
        [Tooltip("The normalized value that the button can be from the `Resting Position` before the button is considered to be resting when not being interacted with.")]
        [Range(0f, 1f)]
        public float restingPositionThreshold = 0.01f;
        [Tooltip("The normalized position of the button between the original position and the pressed position. `0f` will set the button position to the original position, `1f` will set the button position to the pressed position.")]
        [Range(0f, 1f)]
        public float positionTarget = 0f;
        [Tooltip("The amount of force to apply to push the Button towards the intended target position.")]
        ...
        /// </summary>
        /// <returns>Returns `true` if the button is currently at the resting position.</returns>
